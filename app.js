...
let agenda=JSON.parse(localStorage.getItem("v1agenda")||"[]"),ignored=JSON.parse(localStorage.getItem("v1ignored")||"[]"),filter="Todas";
...
function parseAgenda(text){text=(text||"").replace(/\r/g," ").replace(/\n/g," ").replace(/\s+/g," ").trim();let starts=[...text.matchAll(/(\d{1,2}:\d{2})\s*-\s*(\d{1,2}:\d{2})/g)],fresh=[],bad=[];for(let i=0;i<starts.length;i++){let seg=text.slice(starts[i].index,i+1<starts.length?starts[i+1].index:text.length),time=starts[i][1],tm=seg.match(/Tipo:\s*(.*?)\s+Panel:/i),type=tm?tm[1].trim():"",kind=classify(type),pm=seg.match(/Tlf:\s*([+\d][\d\s-]{7,})/i),tel=pm?norm(pm[1]):"",mm=seg.match(/Mto:\s*-?(\d+)/i),mto=mm?mm[1]:"",city="",address="",cp="",a=seg.search(/Panel:/i),b=seg.search(/\sCP:/i);if(a>=0&&b>a){let middle=seg.slice(a,b).replace(/^Panel:\s*/i,"").trim(),parts=middle.split(/\s+-\s+/);if(parts.length>=2){parts.shift();city=parts.shift().trim();address=parts.join(" - ").trim()}}let cm=seg.match(/CP:\s*(\d{5})/i);if(cm)cp=cm[1];city=cityName(city);let id=(mto||tel||time)+"|"+time,old=agenda.find(x=>x.id===id),x={id,time,type,kind,tel,mto,city,address,cp,state:old?.state||"Pendiente",prepared:old?.prepared||false,cancelReason:old?.cancelReason||"",issues:[]};validate(x);kind?fresh.push(x):bad.push({time,type:type||"Sin tipo"})}agenda=fresh;ignored=bad;save();render()}
...
function wa(x,custom){validate(x);if(x.issues.length){render();return alert("Revisa primero: "+x.issues.join(", "))}x.prepared=true;save();render();let c=cfg(),m=custom||fill(x.kind==="Instalación"?c.inst:c.maint,x);location.href=`https://wa.me/34${x.tel.replace(/^34/,"")}?text=${encodeURIComponent(m)}`}
function cancel(x){show("Cancelar cita",`<label>Motivo de cancelación</label><textarea id="reason">${x.cancelReason||""}</textarea><button id="cancelMail" class="red" style="width:100%;margin-top:10px">Cancelar y preparar correo</button>`);$("cancelMail").onclick=()=>{let r=$("reason").value.trim();if(!r)return alert("Indica el motivo.");x.state="Cancelada";x.cancelReason=r;save();render();hide();let c=cfg();location.href=`mailto:${c.to}?cc=${encodeURIComponent(c.cc)}&subject=${encodeURIComponent(fill(c.subject,x,r))}&body=${encodeURIComponent(fill(c.mail,x,r))}`}}
function states(x){show("Cambiar estado",`<div style="display:grid;gap:8px"><button id="sp" class="soft">🟡 Pendiente</button><button id="sc" class="green">🟢 Confirmada</button><button id="sx" class="danger">🔴 Cancelada</button></div>`);$("sp").onclick=()=>{x.state="Pendiente";save();hide();render()};$("sc").onclick=()=>{x.state="Confirmada";save();hide();render()};$("sx").onclick=()=>cancel(x)}
...
function settings(){let c=cfg();show("Ajustes",`<label>Nombre del técnico</label><input id="tech" placeholder="Escribe tu nombre" value="${c.tech}"><hr><label>Palabras de instalación (una por línea)</label><textarea id="keys">${c.keys.join("\n")}</textarea><label>Plantilla instalación</label><textarea id="inst">${c.inst}</textarea><button id="rInst" class="soft">Restaurar predeterminado</button><label>Plantilla mantenimiento</label><textarea id="maint">${c.maint}</textarea><button id="rMaint" class="soft">Restaurar predeterminado</button><hr><label>Correo Para</label><input id="to" value="${c.to}"><label>CC</label><input id="cc" value="${c.cc}"><label>Asunto cancelación</label><input id="subject" value="${c.subject}"><label>Cuerpo cancelación</label><textarea id="mail">${c.mail}</textarea><button id="rMail" class="soft">Restaurar predeterminado</button><div class="row"><button id="saveCfg" class="red">Guardar cambios</button></div>`);
...
function render(){let n=s=>agenda.filter(x=>x.state===s).length;$("stats").innerHTML=`<div class="stats"><div class="stat"><b>${agenda.length}</b><span class="tiny">Citas</span></div><div class="stat"><b>${n("Pendiente")}</b><span class="tiny">Pendientes</span></div><div class="stat"><b>${n("Confirmada")}</b><span class="tiny">Confirmadas</span></div><div class="stat"><b>${n("Cancelada")}</b><span class="tiny">Canceladas</span></div></div>`;
...
$("ignored").innerHTML=filter==="Ignoradas"?`<div class="box">${ignored.length?ignored.map(x=>`<div><b>${x.time}</b> · ${x.type}</div>`).join(""):"No hay citas ignoradas."}</div>`:(ignored.length?`<div class="box"><details><summary>${ignored.length} citas ignoradas</summary>${ignored.map(x=>`<div class="tiny">${x.time} · ${x.type}</div>`).join("")}</details></div>`:"");
let list=agenda;if(filter==="Pendientes")list=agenda.filter(x=>x.state==="Pendiente");if(filter==="Confirmadas")list=agenda.filter(x=>x.state==="Confirmada");if(filter==="Canceladas")list=agenda.filter(x=>x.state==="Cancelada");if(filter==="Ignoradas")list=[];$("results").innerHTML="";
list.forEach(x=>{let d=document.createElement("article"),sc=x.state==="Pendiente"?"pending":x.state==="Confirmada"?"confirmed":"cancelled";d.className="card "+(x.issues.length?"issue ":"")+(x.prepared?"prepared":"");d.innerHTML=`<div class="top"><span class="tag">${x.kind}</span><button class="state ${sc} st">${x.state}</button></div><h2>${x.time} · ${x.city||"Revisar población"}</h2><div>${x.type}</div><div class="muted">Prospecto: ${x.mto||"—"} · ${x.tel||"Sin teléfono"}</div><div style="margin-top:8px">📍 ${x.address||"Dirección no detectada"} ${x.cp||""}</div>${x.prepared?'<div class="pill">✓ WhatsApp preparado</div>':""}${x.issues.map(i=>`<div class="pill">⚠️ ${i}</div>`).join("")}<div class="actions"><button class="green w">WhatsApp</button><button class="soft call">Llamar</button><button class="soft maps">Maps</button></div><div class="row"><button class="soft edit">Editar</button><button class="soft review">Revisar mensaje</button><button class="soft change">Cambiar estado</button></div>`;d.querySelector(".st").onclick=()=>states(x);d.querySelector(".w").onclick=()=>wa(x);d.querySelector(".call").onclick=()=>location.href="tel:"+x.tel;d.querySelector(".maps").onclick=()=>location.href="https://www.google.com/maps/search/?api=1&query="+encodeURIComponent([x.address,x.cp,x.city].filter(Boolean).join(", "));d.querySelector(".edit").onclick=()=>edit(x);d.querySelector(".review").onclick=()=>preview(x);d.querySelector(".change").onclick=()=>states(x);$("results").appendChild(d)});
...
$("settings").onclick=settings;$("mclose").onclick=hide;$("modal").onclick=e=>{if(e.target===$("modal"))hide()};$("manualBtn").onclick=()=>{let h=$("manual").classList.toggle("hidden");$("manualBtn").textContent=h?"Entrada manual ▾":"Entrada manual ▴"};$("process").onclick=()=>parseAgenda($("raw").value);$("paste").onclick=async()=>{try{$("raw").value=await navigator.clipboard.readText();parseAgenda($("raw").value)}catch(e){alert("Mantén pulsado y toca Pegar.")}};$("newAgenda").onclick=()=>{if(confirm("¿Borrar la agenda guardada y sus estados?")){agenda=[];ignored=[];save();$("manual").classList.remove("hidden");$("manualBtn").textContent="Entrada manual ▴";render()}};
let q=new URLSearchParams(location.search),shared=q.get("text")||[q.get("title"),q.get("url")].filter(Boolean).join("\n");if(shared){$("raw").value=shared;parseAgenda(shared);history.replaceState({},"","./")}else render();if("serviceWorker"in navigator)navigator.serviceWorker.register("./sw.js");
