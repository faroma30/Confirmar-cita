...
function wa(x,custom){validate(x);if(x.issues.length){render();return alert("Revisa primero: "+x.issues.join(", "))}x.prepared=true;save();render();let c=cfg(),m=custom||fill(x.kind==="Instalación"?c.inst:c.maint,x);openExternal(`https://wa.me/34${x.tel.replace(/^34/,"")}?text=${encodeURIComponent(m)}`)}
...
function render(){
 ...
 $("stats").innerHTML=`<div class="stats"><div class="stat"><b>${agenda.length}</b><span>Citas</span></div><div class="stat"><b>${n("Pendiente")}</b><span>Pendientes</span></div><div class="stat"><b>${n("Confirmada")}</b><span>Confirmadas</span></div><div class="stat"><b>${n("Cancelada")}</b><span>Canceladas</span></div></div>`;
 ...
 d.querySelector(".st").onclick=()=>states(x);d.querySelector(".w").onclick=()=>wa(x);d.querySelector(".callb").onclick=()=>location.href="tel:"+x.tel;d.querySelector(".mapsb").onclick=()=>openExternal("https://www.google.com/maps/search/?api=1&query="+encodeURIComponent(mapAddress(x.address,x.city)));d.querySelector(".edit").onclick=()=>edit(x);d.querySelector(".review").onclick=()=>preview(x);d.querySelector(".change").onclick=()=>states(x);$("results").appendChild(d)});
