...
function wa(x,custom){validate(x);if(x.issues.length){render();return alert("Revisa primero: "+x.issues.join(", "))}x.prepared=true;save();render();let c=cfg(),m=custom||fill(x.kind==="Instalación"?c.inst:c.maint,x);openWhatsAppDirect(x.tel,m)}
