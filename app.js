...
function savedDateNav(){let today=TODAY(),tomorrow=TOMORROW(),d0=new Date(today+"T12:00:00");d0.setDate(d0.getDate()-1);let yesterday=d0.toISOString().slice(0,10),saved=Object.keys(stores).sort().filter(d=>![yesterday,today,tomorrow].includes(d)),ds=[yesterday,today,tomorrow,...saved];return `<div class="datechips" id="datechips">${ds.map(d=>`<button class="secondary datechip ${d===activeDate?"on":""} ${d===today?"today":""}" data-d="${d}">${shortDate(d)}</button>`).join("")}<button class="secondary datechip addDate">＋</button></div><div class="daytitle"><span>${fmtDate(activeDate)}</span><button class="deleteDay">🗑️ Borrar día</button></div>`}
function centerTodayChip(){let strip=document.getElementById("datechips"),today=strip?.querySelector(".datechip.today");if(strip&&today){let left=today.offsetLeft-(strip.clientWidth-today.offsetWidth)/2;strip.scrollTo({left:Math.max(0,left),behavior:"auto"})}}
...
$("stats").innerHTML=savedDateNav()+`<div class="stats stats5"><div class="stat"><b>${visibleAgenda.length}</b><span>Citas</span></div><div class="stat"><b>${contacted}</b><span>Contactadas</span></div><div class="stat"><b>${n("Pendiente")}</b><span>Pendientes</span></div><div class="stat"><b>${n("Confirmada")}</b><span>Confirmadas</span></div><div class="stat"><b>${n("Cancelada")}</b><span>Canceladas</span></div></div>`;
 document.querySelectorAll(".datechip[data-d]").forEach(b=>b.onclick=()=>switchDate(b.dataset.d));centerTodayChip();let add=document.querySelector(".addDate");if(add)add.onclick=chooseNewDate;let del=document.querySelector(".deleteDay");if(del)del.onclick=()=>{if(!confirm(`¿Borrar todo el historial de ${fmtDate(activeDate)}?\n\nSe eliminarán citas, estados, teléfonos, direcciones, notas y motivos de cancelación de ese día.`))return;delete stores[activeDate];agenda=[];ignored=[];importChanges=[];localStorage.setItem("v1datedAgendas",JSON.stringify(stores));render()};
